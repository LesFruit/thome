...
def _setup_account_with_transactions(client):
    ...
    client.post("/api/v1/auth/signup", json={"email": "stmt@example.com", "password": "StrongPass1!"})
    login = client.post("/api/v1/auth/login", json={"email": "stmt@example.com", "password": "StrongPass1!"})
    token = login.json()["access_token"]
    headers = {"Authorization": f"Bearer {token}"}

    client.post("/api/v1/holders", json={
        "first_name": "Stmt", "last_name": "User", "date_of_birth": "1990-01-01",
    }, headers=headers)
    ...
    client.post(f"/api/v1/accounts/{a1['id']}/deposit", json={"amount_cents": 100000}, headers=headers)
...
def test_generate_statement(client):
    ...
    resp = client.post(f"/api/v1/accounts/{acct_id}/statements", json={
...
def test_list_statements(client):
    ...
    client.post(f"/api/v1/accounts/{acct_id}/statements", json={
        "start_date": "2020-01-01", "end_date": today,
    }, headers=headers)
    client.post(f"/api/v1/accounts/{acct_id}/statements", json={
        "start_date": "2020-06-01", "end_date": today,
    }, headers=headers)
...
def test_get_statement(client):
    headers, acct_id, _ = _setup_account_with_transactions(client)
    today = date.today().isoformat()
    create = client.post(f"/api/v1/accounts/{acct_id}/statements", json={
        "start_date": "2020-01-01", "end_date": today,
    }, headers=headers)
