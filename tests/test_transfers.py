...
def _setup_two_accounts(client):
    ...
    client.post(
        "/api/v1/auth/signup", json={"email": "xfer@example.com", "password": "StrongPass1!"}
    )
    login = client.post(
        "/api/v1/auth/login", json={"email": "xfer@example.com", "password": "StrongPass1!"}
    )
    ...
    client.post(
        f"/api/v1/accounts/{a1['id']}/deposit", json={"amount_cents": 10000}, headers=headers
    )
