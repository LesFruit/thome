"""Stress and concurrency tests — section 5 of REQUIREMENTS.MD.
...
_ConcurrentSessionLocal = sessionmaker(
    autocommit=False, autoflush=False, bind=_concurrent_engine,
)
...
def _create_funded_accounts(
    client: TestClient, headers: dict, balance: int = 100_000,
):
    """Return (acct1_id, acct2_id) with acct1 funded."""
    client.post("/api/v1/holders", json={
        "first_name": "Stress", "last_name": "Tester",
        "date_of_birth": "1990-01-01",
    }, headers=headers)
    a1 = client.post(
        "/api/v1/accounts", json={"account_type": "checking"},
        headers=headers,
    ).json()
    a2 = client.post(
        "/api/v1/accounts", json={"account_type": "savings"},
        headers=headers,
    ).json()
    client.post(
        f"/api/v1/accounts/{a1['id']}/deposit",
        json={"amount_cents": balance}, headers=headers,
    )
    return a1["id"], a2["id"]
...
def test_concurrent_transfers_no_overdraft(stress_client):
    ...
    def _transfer():
        resp = stress_client.post("/api/v1/transfers", json={
...
def test_concurrent_card_spend_no_overdraft(stress_client):
    ...
    card = stress_client.post(
        f"/api/v1/accounts/{src}/cards", headers=headers,
    ).json()
    ...
    def _spend():
        resp = stress_client.post(
            f"/api/v1/cards/{card['id']}/spend", json={
                "amount_cents": 15_000,
...
def test_idempotency_replay_returns_original(client):
    ...
    resp1 = client.post("/api/v1/transfers", json={
        "source_account_id": src,
        "destination_account_id": dst,
        "amount_cents": 10_000,
...
def test_ledger_invariant_under_load(client):
    ...
    txns_src = client.get(
        f"/api/v1/accounts/{src}/transactions", headers=headers,
    ).json()
    txns_dst = client.get(
        f"/api/v1/accounts/{dst}/transactions", headers=headers,
    ).json()

    total_debits = sum(
        t["amount_cents"] for t in txns_src if t["type"] == "debit"
    )
    total_credits = sum(
        t["amount_cents"] for t in txns_dst if t["type"] == "credit"
    )
...
def test_rapid_signup_uniqueness(client):
    ...
    first = client.post("/api/v1/auth/signup", json={
        "email": "race@example.com", "password": "StrongPass1!",
    })
...
def test_card_spend_idempotency_replay(client):
    ...
    resp1 = client.post(f"/api/v1/cards/{card['id']}/spend", json={
        "amount_cents": 5_000,
        "merchant": "Coffee",
        "idempotency_key": idem_key,
    }, headers=headers)
    ...
    resp2 = client.post(f"/api/v1/cards/{card['id']}/spend", json={
        "amount_cents": 5_000,
        "merchant": "Coffee",
        "idempotency_key": idem_key,
    }, headers=headers)
