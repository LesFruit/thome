...
def test_create_holder_future_dob_rejected(client):
    token = _signup_and_login(client, "future-holder@example.com")
...
def test_create_holder_empty_name_rejected(client):
    token = _signup_and_login(client, "blank-holder@example.com")
    resp = client.post(
        "/api/v1/holders",
        json={
            "first_name": "   ",
...
def test_get_account(client):
    ...
    create = client.post(
        "/api/v1/accounts", json={"account_type": "checking"}, headers=_auth(token)
    )
...
def test_cross_user_account_access_denied(client):
    ...
    create = client.post(
        "/api/v1/accounts", json={"account_type": "checking"}, headers=_auth(token1)
    )
    acct_id = create.json()["id"]
...
def test_freeze_account(client):
    ...
    acct_id = create.json()["id"]
    resp = client.patch(
        f"/api/v1/accounts/{acct_id}", json={"status": "frozen"}, headers=_auth(token)
    )
    assert resp.status_code == 200
...
def test_invalid_status_transition(client):
    ...
    resp = client.patch(
        f"/api/v1/accounts/{acct_id}", json={"status": "active"}, headers=_auth(token)
    )
    assert resp.status_code == 400
